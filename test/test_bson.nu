;; test_bson.nu
;;  tests for RadBSON representation.
;;
...
(class TestBSON is NuTestCase
     ...
     (- testRoundtrip is
        (set x (dict a:"a"
                     ...
                     h:nil))
        (set bson (RadBSON bsonWithDictionary:x))
        (set data (bson dataRepresentation))
        (set bson2 (RadBSON bsonWithData:data))
        ...
        ;; now stay as bson
        (set z (RadBSON bsonWithDictionary:y))
     ...
     (- testOIDs is
        (10 times:
            (do (i)
                (set id1 ((RadBSONObjectID objectID)))
                (set id2 ((RadBSONObjectID alloc)
                          initWithData:(id1 dataRepresentation)))
                (assert_equal id1 id2)
                
                (set id3 (RadBSONObjectID new))
                (set id4 (RadBSONObjectID new))
     ...
     (- testCode is
        (set code '(do (x) (+ x 1)))
        (set bson (RadBSON bsonWithDictionary:(dict code:code)))
